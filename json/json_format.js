...
//json格式化后显示的标签id为自定义
function ProcessFormat(response,documentId){
    SetTab();
    var json = response;
    var html = "";
    try{
        ...
        $id(documentId).innerHTML = "<PRE class='CodeContainer'>"+html+"</PRE>";
    }catch(e){
        $id(documentId).innerHTML = "<PRE class='CodeContainer'>"+response+"</PRE>";
        //alert("JSON数据格式不正确:\n"+e.message);
    }
}

//json格式化后显示的标签id为Canvas
function Process(response) {
    ...
    try {
        ...
        $id("Canvas").innerHTML = "<PRE class='CodeContainer'>" + response + "</PRE>";
        //alert("JSON数据格式不正确:\n" + e.message);
    }
}
...
var str="";
    ...
    if (style != 'Null') {
         str = "<span   class='" + style + "'>" + quote + literal + quote + comma + "</span>" + "<span style='display:none' name='style'  class='" + style + "Style'>" + style + "</span>";
    } else {
         str = "<span  class='" + style + "'>" + quote + literal + quote + comma + "</span>";
    }

    str=str+"<input type='text' placeholder='暂无注释' style='display:none'  name='responseParamDescriptions'  class='returnparam'>" ;
